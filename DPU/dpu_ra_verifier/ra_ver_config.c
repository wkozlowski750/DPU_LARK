...
int prover_props_init(const char *name, const char *starting_addr, uint32_t quant) {
	...
	// start_times = rte_calloc("start_table", (quant), sizeof(struct timespec), 0);
	// if (start_times == NULL) {
	// 	printf("Failed to allocate memory for start_times. \n");
	// 	return -2;
	// }
	// stop_times = rte_calloc("stop_table", (quant), sizeof(struct timespec), 0);
	// if (stop_times == NULL) {
	// 	printf("Failed to allocate memory for stop_times. \n");
	// 	return -2;
	// }

	start_times = rte_calloc("start_table", (quant), sizeof(uint64_t), 0);
	if (start_times == NULL) {
		printf("Failed to allocate memory for start_times. \n");
		return -2;
	}
	stop_times = rte_calloc("stop_table", (quant), sizeof(uint64_t), 0);
	if (stop_times == NULL) {
		printf("Failed to allocate memory for stop_times. \n");
		return -2;
	}
	...
	for (uint32_t i = 0; i < (quant); i++) {
		...
		prover->valid = false;
		// prover->start_time.tv_nsec = 0;
		// prover->start_time.tv_sec = 0;
		// prover->stop_time.tv_sec = 0;
		// prover->stop_time.tv_nsec = 0;
