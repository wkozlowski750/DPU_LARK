			  ...
			  union doca_data ctx_user_data)
{
	...
	*result = DOCA_SUCCESS;
	...
	long long diff = calculate_elapsed_time_ns(start_time, stop_time);
	printf("Time in ns: %lld\n", diff);
	FILE *fp = fopen("benchmark_times.csv", "a"); // Open the file in append mode
    if (fp == NULL) {
        perror("Failed to open file");
        return 1;
    }
    fprintf(fp, "%d,%lld\n", resources->bytes, diff);
    fclose(fp);
	/* Free task */
 ...
 */
doca_error_t
sha_create(char *src_buffer, int bytes)
{
