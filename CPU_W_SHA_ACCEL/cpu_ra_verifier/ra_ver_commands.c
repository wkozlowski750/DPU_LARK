...
void cmd_attest_parsed(void *parsed_result, struct cmdline *cl, __rte_unused void *data) {
	...
	uint16_t port;
	char tmp[256];
	sprintf(tmp, "../results/%u_prov/cpu_ra_test_%ucore_%uprover.csv", prover_quant, rte_lcore_count(), prover_quant);
	...
	*ctr++;
	...
	RTE_ETH_FOREACH_DEV(port) {
		for (uint8_t test_number = 0; test_number < 1; test_number++) {
			...
			while (1) {
				...
				uint32_t tot_completions = 0;
				for (int i = 0; i < 64; i++) {
					if (ver_ctrs[i] == NULL) continue;
					tot_completions += *ver_ctrs[i];
				}
				if ((tot_completions >= prover_quant - 2) && !got_final_time) {

					clock_gettime(CLOCK_MONOTONIC, &final_stop_time);
					got_final_time = true;
					printf("--------------------------------------\n");
					printf("Got final time. Total completions: %u", tot_completions);
					printf("--------------------------------------\n");
				}
				...
				// 	// if (clock_gettime(CLOCK_MONOTONIC, &start_times_mbuf[mbuf_index]) != 0) {
			...
			long long diff = calculate_elapsed_time_ns(first_start_time, final_stop_time);
			printf("Main: total verification time: %lld\n", diff);
		}
	}
}
